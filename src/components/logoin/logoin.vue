...
<van-form  v-if="isshowfrom2">
  ...
  <button class="btn  top" v-if="ishowva"  @click="clickvadate" :disabled="disabled">{{textValue}}</button>
  <div style="margin: 16px;">
    <van-button round block type="danger " native-type="submit" @click="submit2" >
       验证码登录
    </van-button>
  </div>
</van-form>
...
export default {
 ...
 async submit(){
   console.log('用户点击了')
   const  {data:res}=await api.getloginmethod(this.phone,this.password)
 ...
 async submit2(){
    ...
    if(this.phone1!==''&&this.password1!==''){
      const  {data:res}=await api.sceodvatatemethod(phones,captcha)
       if(res.code==200){
         Toast({
           message:'登录成功'
         })
