<template>
 <div>
    ...
    <div class="comment-warper"  v-for="(item,index) in commentdata" :key="index">
        <div class="left-img">
             <img  :src="item.user.avatarUrl"  alt="">
        </div>
        ...
        <!-- 右边 -->
        <div class="right-box">
                  <div class="top_Box" style="font-size:14px;color:#999;line-heigt:20px">
                      <div class="top_conetboxs">
                        <div class="lineh">{{item.user.nickname}}</div>
                        <div>
                            <span style="margin-right:3px;font-size:13px;vertical-align: top;" v-show="item.likedCount>0">{{item.likedCount}}</span>
